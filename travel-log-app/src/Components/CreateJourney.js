...
const CreateJourney = (props) => { 
  ...
  const addJourni = async ()=>{
    ...
    try {
      let response = await axios.post('https://fynd-project-backend.vercel.app/add', {userData});
    } catch (error) {
    ...
    return (
        <>
            <div className='moment-page'>
                ...
                <form action='https://fynd-project-backend.vercel.app/album' method='post' name='Endgame' enctype='multipart/form-data'>
  ...
  const TitleContent = () => {
    ...
    const addTitle = async ()=>{
        ...
        try {
            let response = await axios.post('https://fynd-project-backend.vercel.app/upload', formData,{headers: {"Content-Type": "multipart/form-data"}});
        } catch (error) {
            console.error('Error sending username:', error);
          }
    }
